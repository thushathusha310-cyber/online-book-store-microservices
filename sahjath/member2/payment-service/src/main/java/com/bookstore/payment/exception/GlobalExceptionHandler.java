...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> handleInvalid(InvalidPaymentException ex, HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), request.getRequestURI(), Map.of());
    }

    @ExceptionHandler(DownstreamServiceException.class)
    ResponseEntity<ApiError> handleDownstream(DownstreamServiceException ex, HttpServletRequest request) {
